...
class user_interface():
    ...
    def analysis(self):
        self.print_options(self.population.columns[2:],0)
        indep = int(input("Select X (Independent) Variable:"))
        dep = int(input("Select Y (Dependent) Variable:"))
        X = [i[indep-2] for i in self.population.data]
        Y = [i[dep-2] for i in self.population.data]
        analysis = regression(X,Y,self.population.columns[indep-1],self.population.columns[dep-1])
        analysis.plot()
        analysis.SLR()
